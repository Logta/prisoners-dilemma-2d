...
impl RouletteSelection {
    ...
    pub fn select_parents_with_penalty(agents: &HashMap<Uuid, Agent>, penalty_rate: f32) -> Vec<Agent> {
        let mut rng = rand::thread_rng();
        let mut selected = Vec::new();
        ...
        let adjusted_scores: Vec<f32> = agents_vec
            ...
            .map(|a| {
                ...
                match a.strategy {
                    StrategyType::TitForTat | StrategyType::Pavlov => base_score * penalty_multiplier,
                    _ => base_score,
                }
            })
            .collect();
