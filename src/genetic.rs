...
pub fn select_agents(agents: &[Agent], method: &SelectionMethod, count: usize) -> Vec<Agent> {
    match method {
        ...
        SelectionMethod::RouletteWheel => {
            ...
            let adjusted_scores: Vec<f64> = agents
                .iter()
...
pub fn replace_generation(
    ...
    // 選択する親の数（偶数にする）
    let parent_count = if population_size % 2 == 0 {
        population_size
    } else {
        population_size + 1
    };
...
mod tests {
    ...
    fn test_one_point_crossover() {
        ...
        // 子は親の特性を継承している
        assert!(
            child1.cooperation_rate == parent1.cooperation_rate
                || child1.cooperation_rate == parent2.cooperation_rate
        );
        assert!(
            child1.movement_rate == parent1.movement_rate
                || child1.movement_rate == parent2.movement_rate
        );
        assert!(
            child2.cooperation_rate == parent1.cooperation_rate
                || child2.cooperation_rate == parent2.cooperation_rate
        );
        assert!(
            child2.movement_rate == parent1.movement_rate
                || child2.movement_rate == parent2.movement_rate
        );
    }
